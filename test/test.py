...
async def test_basic_operation(dut):
    ...
    # Configure: alpha = 2
    dut.ui_in.value = (0b11 << 5) | 0  # Mode=11, config alpha
    dut.uio_in.value = 2
    ...
    for cell in hot_cells:
        dut.ui_in.value = (0b01 << 5) | cell  # Mode=01, write
    ...
    dut.ui_in.value = (0b10 << 5) | 0  # Mode=10, read cell 0 (corner)
    await ClockCycles(dut.clk, 2)
    edge_temp = int(dut.uio_out.value) & 0x0F
    
    dut._log.info(f"✓ Edge temperature: {edge_temp}")
    assert edge_temp < 4, "Edge should be cool"
...
async def test_boundary_conditions(dut):
    ...
    dut.ui_in.value = (0b11 << 5) | 1  # Mode=11, config boundary
    dut.uio_in.value = 5
    await ClockCycles(dut.clk, 2)
    ...
    dut.ui_in.value = (0b01 << 5) | 14
    dut.uio_in.value = 15
    await ClockCycles(dut.clk, 2)
    
    # Run simulation
    dut.ui_in.value = 0b00 << 5
    await ClockCycles(dut.clk, 36 * 20)
    
    # Read edge cell
    dut.ui_in.value = (0b10 << 5) | 0
    await ClockCycles(dut.clk, 2)
    edge_temp = int(dut.uio_out.value) & 0x0F
...
async def test_read_write(dut):
    ...
    for addr, value in test_data:
        # Write
        dut.ui_in.value = (0b01 << 5) | addr
        dut.uio_in.value = value
        await ClockCycles(dut.clk, 2)
        
        # Read back
        dut.ui_in.value = (0b10 << 5) | addr
        await ClockCycles(dut.clk, 2)
